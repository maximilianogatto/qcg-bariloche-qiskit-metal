...
class Frame(QComponent):
    ...
    def make(self):
        geoms, background = self._make_border()
        
        # aplly global transformations
    ...
    def _make_border(self):
        ...
        # Create the borders
        line_top = draw.LineString([[-frame_lx/2, frame_ly/2], [frame_lx/2, frame_ly/2]])
        line_bottom = draw.LineString([[-frame_lx/2, -frame_ly/2], [frame_lx/2, -frame_ly/2]])
        line_left = draw.LineString([[-frame_lx/2, frame_ly/2], [-frame_lx/2, -frame_ly/2]])
        line_right = draw.LineString([[frame_lx/2, frame_ly/2], [frame_lx/2, -frame_ly/2]])
        
        # rounded point in the corners
        p_top_left = draw.Point([-frame_lx/2, frame_ly/2]).buffer(f_width/2, cap_style=1, join_style=2)
        p_top_right = draw.Point([frame_lx/2, frame_ly/2]).buffer(f_width/2, cap_style=1, join_style=2)
        p_bottom_left = draw.Point([-frame_lx/2, -frame_ly/2]).buffer(f_width/2, cap_style=1, join_style=2)
        p_bottom_right = draw.Point([frame_lx/2, -frame_ly/2]).buffer(f_width/2, cap_style=1, join_style=2)

        border = draw.union(line_top, line_bottom, line_left, line_right)
        border = border.buffer(f_width/2, cap_style=2, join_style=2)
        border = draw.union(border, p_top_left, p_top_right, p_bottom_left, p_bottom_right)
        
        # make the background
        background = border.buffer(f_gap, cap_style=2, join_style=2)
        
        
        return {'lines': border}, background
    
    def _emit_qgeometry(self, geoms, background):
        # TODO: modify depending on the geometries emmited
        p = self.p
        # background (pocket)
        self.add_qgeometry('poly', {'background': background}, subtract=True, chip=p.chip)

        # Pads, loop
        for name in geoms.keys():
            self.add_qgeometry('poly', {name: geoms[name]}, layer=p.layer)
